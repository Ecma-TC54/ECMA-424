...
export class Ecmarkup {
  ...
  openSection(id, isDeprecated) {
    ...
    let legacy = (isDeprecated && isDeprecated === true) ? "legacy" : "";
  ...
  addLocation(breadcrumb) {
    if (breadcrumb) {
      this.append(`<br><strong>Location:</strong> ${breadcrumb}<br>\n`);
    }
    return this;
  }
  ...
  addProperties(schemaObject, appendedDescription) {
    ...
    if (schemaObject.pattern) {
      this.append(`<strong>Pattern Constraint:</strong> <pre><code>${schemaObject.pattern}</code></pre><br>\n`);
    }
    ...
    if (schemaObject.description) {
      ...
      description = this.processMarkdownLinks(description);
  ...
  addEnums(enumValues, metaEnum, tableCounter) {
    if (enumValues && metaEnum) {
      ...
      for (const row of rows) {
        const processedDescription = this.processMarkdownLinks(row[1]);
  ...
  addPropertyTableRow(key, type, requiredLabel, description) {
    const processedDescription = this.processMarkdownLinks(description);
    this.append(`<tr><td>${key}</td><td>${type}</td><td>${requiredLabel}</td><td>${processedDescription}</td></tr>\n`);
    return this;
  }
  ...
  openArraySection(breadcrumb, title, propertyName, requiredLabel, type, description, appendedDescription, isDeprecated) {
    const id = `sec-${breadcrumb.replace(/[^a-zA-Z0-9\-]+/g, "-").toLowerCase()}`;
    this.openSection(id, isDeprecated);
    ...
    if (description) {
      ...
      desc = this.processMarkdownLinks(desc);
  ...
  addNumberedBullet(value) {
    if (value) {
      const translatedValue = EnglishTranslation.translateToUK(value);
