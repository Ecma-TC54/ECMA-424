...
export class Ecmarkup {
  ...
  addProperties(schemaObject, appendedDescription) {
    ...
    if (schemaObject.typeLabel || schemaObject.type) {
      this.append(`<strong>Type:</strong> ${schemaObject.typeLabel ? schemaObject.typeLabel : this.sentenceCase(schemaObject.type)}<br>\n`);
    }
  ...
  addEnums(enumValues, metaEnum, tableCounter) {
    if (enumValues && metaEnum) {
      ...
      for (const row of rows) {
        ...
        this.append(`<tr><td>${row[0]}</td><td>${processedDescription}</td></tr>\n`);
      }
      this.append(`</table>\n`);
      this.append(`</emu-table>\n`);
    } else if (enumValues) {
