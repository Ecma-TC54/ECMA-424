...
export class EcmarkupGenerator {
  ...
  processPropertyByType(rso, key, breadcrumb, fromArray) {
    ...
    if (rso.type === "array") {
      ...
      throw new Error("JSON allOf is not currently supported. Generated documentation may be inaccurate.");
    } else {
      this.processJsonProperty(rso, key, newBreadcrumb, fromArray);
    }
  }
  ...
  processJsonObject(schemaObject, propertyName, breadcrumb, fromArray) {
    ...
    // Add properties table if this object has properties
    if (!imposter && so.properties) {
      this.md.addPropertiesTable(breadcrumb, propertyName, so.properties, this.tableCounter++);

      for (let key in so.properties) {
        if (!key.startsWith("$")) {
          let rso = so.properties[key]["$ref"] ? this.resolveSchemaDefinitionFromRef(so.properties[key]["$ref"].split("/").pop()) : so.properties[key];
          rso.requiredLabel = so.properties[key].requiredLabel;
          let type = this.determinePropertyType(rso);
          let description = so.properties[key].description;
      ...
      for (let key in so.properties) {
        if (!key.startsWith("$")) {
          ...
          let rso = so.properties[key]["$ref"] ? this.resolveSchemaDefinitionFromRef(so.properties[key]["$ref"].split("/").pop()) : so.properties[key];
          rso.imposter = this.isImposter(so.properties[key], this.addCrumb(breadcrumb, key));
          rso.requiredLabel = so.properties[key].requiredLabel;
          rso.origin = so.properties[key].origin;
          rso.title = (so.properties[key].title) ? so.properties[key].title : rso.title;
          rso.description = (so.properties[key].description) ? so.properties[key].description : rso.description;
          rso.examples = (so.properties[key].examples) ? so.properties[key].examples : rso.examples;
  ...
  processJsonProperty(so, propertyName, breadcrumb, fromArray, appendedDescription, omitLocation) {
    ...
    this.md.openPropertySection(breadcrumb, so.title, propertyName, so, fromArray, appendedDescription, omitLocation);
  ...
  createArrayItemDescription(itemSchema) {
    ...
    if (itemSchema.type && itemSchema.type !== "array" && itemSchema.type !== "object") {
      return `Each item of this array must be ${this.chooseArticle(itemSchema.type)} ${itemSchema.type}.`;
    } else if (itemSchema.title) {
      return `Each item of this array must be ${this.chooseArticle(itemSchema.title)} ${itemSchema.title} object.`;
    }
  ...
  processJsonArray(so, propertyName, breadcrumb) {
    ...
    if (so.items && so.items.type && (so.items.type !== "array" && so.items.type !== "object")) {
      ...
      tempSo.typeLabel = tempSo.type + " (of " + this.sentenceCase(tempSo.items.type) + ")";
    ...
    if (so.uniqueItems && so.uniqueItems === true) {
      this.md.addImportantNote("All items must be unique.");
    }
  }
